...
void stream::feed(shared_ptr<datagram> new_datagram) {
    ...
    if (it != recv_datagram_queue_.end() && it->first == recv_offset_) {
        ...
        do {
            ...
            output_ring.put(
                boost::make_shared<recv_data>(peer_.addr(), id_, recv_data_));
...
void stream::send(const std::string &data) {
    ...
    while (offset < data.size()) {
        ...
        size_t seg_length
            = left_length > segment_max_ ? segment_max_ : left_length;
        ...
        send_datagram_queue_.insert(
            datagram_map_type::value_type(new_datagram->offset_, new_datagram));
...
void stream::send_one_datagram() {
    ...
    *copy = *to_send;
    ...
    timer.async_wait(
        bind(&stream::retransmit, this, asio::placeholders::error()));
}
