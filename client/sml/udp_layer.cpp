...
udp_layer::udp_layer(service &a_service)
    ...
    , default_filter_(
          [](shared_ptr<std::string>, const address &) { return true; }) {
    start_receive();
}
...
void udp_layer::start_receive() {
    socket_.async_receive_from(boost::asio::buffer(recv_buffer_),
        remote_endpoint_,
        boost::bind(&udp_layer::handle_receive, this,
            boost::asio::placeholders::error,
            boost::asio::placeholders::bytes_transferred));
}

void udp_layer::handle_receive(
    const boost::system::error_code &error, std::size_t bytes_transferred) {
    if (!error || error == boost::asio::error::message_size) {
        shared_ptr<std::string> received_message(
        ...
        if (feed_peer) {
            ...
            if (peer_it == peer_map_.end()) {
                peer_map_.insert(peer_map_type::value_type(addr,
                    make_shared<peer>(service_.io_context(), *this, addr)));
...
void udp_layer::send_to(boost::shared_ptr<std::string> msg, const address &addr,
    handler_type handler) {
    boost::asio::ip::udp::endpoint endpoint(
        boost::asio::ip::address::from_string(addr.ip()), addr.port());
    socket_.async_send_to(boost::asio::buffer(*msg), endpoint,
        boost::bind(&udp_layer::handle_send, this,
            boost::asio::placeholders::error, msg, addr, handler));
}
...
void udp_layer::add_peer(const address &addr) {
    peer_map_.insert(peer_map_type::value_type(
        addr, make_shared<peer>(service_.io_context(), *this, addr)));
}
...
void udp_layer::handle_send(const boost::system::error_code &error,
