...
service::service(asio::io_context &io_context, const configuration &conf)
    ...
    udp_layer_.register_filter(
        bind(&service::server_datagram_filter, this, _1, _2), conf_.server());
    timer_.expires_from_now(boost::posix_time::seconds(0));
    timer_.async_wait(
        bind(&service::query_server, this, boost::asio::placeholders::error()));
}
...
void service::query_server(const boost::system::error_code &ec) {
    if (!ec) {
        ...
        timer_.async_wait(bind(
            &service::query_server, this, boost::asio::placeholders::error()));

        asio::ip::udp::resolver resolver(io_context_);
        asio::ip::udp::resolver::query query(asio::ip::host_name(), "");
        asio::ip::udp::resolver::iterator it = resolver.resolve(query);
        udp_layer_.send_to(
            boost::make_shared<std::string>(it->endpoint().address().to_string()
                + ":" + std::to_string(conf_.port())),
            conf_.server(), [](shared_ptr<std::string>, const address &) {});
    }
}

bool service::server_datagram_filter(
