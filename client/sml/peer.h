...
public:
    ...
    peer(asio::io_context &io_context, udp_layer &udp_layer,
        const address &addr);
...
private:
    ...
    const static uint32_t hello_complete
        = sent_hello | received_hello_ack | received_hello;
    ...
    const static uint32_t public_key_exchanged
        = sent_public_key | received_public_key | received_public_key_ack;
    const static uint32_t shared_key_agreement = 1U << 6;
    const static uint32_t established
        = hello_complete | public_key_exchanged | shared_key_agreement;
