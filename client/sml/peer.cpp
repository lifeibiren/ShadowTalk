...
namespace sml {
peer::peer(
    asio::io_context &io_context, udp_layer &a_udp_layer, const address &addr)
    ...
    //        encrypt_layer::algorithm::AES_128,
    ...
    //    CryptoPP::Integer priv(udp_layer_.conf().private_key().c_str()),
    //    pub(udp_layer_.conf().public_key().c_str()); std::cout << "spriv " <<
    //    std::hex << priv << std::endl; std::cout << "spub " << std::hex << pub
    //    << std::endl; std::unique_ptr<byte[]> priv_blk(new
    //    byte[priv.ByteCount()]); std::unique_ptr<byte[]> pub_blk(new
    //    byte[pub.ByteCount()]); priv.Encode(priv_blk.get(), priv.ByteCount());
    //    pub.Encode(pub_blk.get(), pub.ByteCount());
    //    dh_key_agreement_.set_static_key_pairs(std::string((const char
    //    *)priv_blk.get(), priv.ByteCount()),
    //        std::string((const char *)pub_blk.get(), pub.ByteCount()));

    // peer_spub_ = std::string((const char *)pub_blk.get(), pub.ByteCount());

    dh_key_agreement_.set_static_key_pairs(
        hex_str_to_bytes(udp_layer_.conf().private_key()),
...
void peer::generate_shared_key() {
    ...
    encrypt_layer_ = boost::make_shared<encrypt_layer>(
        encrypt_layer::algorithm::AES_128, key_);
}
...
void peer::send_hello() {
    send_raw_datagram_with_retransmit(
        datagram::create_hello(udp_layer_.conf().id()));
...
void peer::received_hello_handler(shared_ptr<datagram> new_datagram) {
    ...
    peer_spub_
        = hex_str_to_bytes(udp_layer_.conf().get_peer_public_key_by_id(id_));
...
void peer::hello_complete_handler() {
    send_public_key();
    datagram_handler_
        = boost::bind(&peer::public_key_exchange_handler, this, _1);
    log << "hello complete\n";
}
...
void peer::established_handler(shared_ptr<datagram> new_datagram) {
    ...
    if (it == stream_map_.end()) {
        stream_map_.insert(stream_map_type::value_type(new_datagram->id_,
            make_shared<stream>(
                stream(io_context_, *this, new_datagram->id_))));
...
void peer::retransmit_raw_datagram(
    shared_ptr<datagram> msg, const system::error_code &ec) {
