...
const CryptoPP::Integer dh_key_agreement::ref_p(
...
const CryptoPP::Integer dh_key_agreement::ref_g(
...
const CryptoPP::Integer dh_key_agreement::ref_q(
...
inline std::unique_ptr<CryptoPP::SecByteBlock> convert_bytes_to_SecByteBlock(
    const std::string bytes) {
    return boost::make_unique<CryptoPP::SecByteBlock>(
        (const byte *)bytes.c_str(), bytes.size());
}
...
void dh_key_agreement::generate_ephemeral_key_pairs() {
    epriv
        = make_unique<CryptoPP::SecByteBlock>(dh2->EphemeralPrivateKeyLength());
...
void dh_key_agreement::set_static_key_pairs(
    const std::string &static_priv_key_bytes,
    ...
    //        (const byte *)static_priv_key_bytes.c_str(),
    //        static_priv_key_bytes.size());
    //    spub = boost::make_unique<CryptoPP::SecByteBlock>(
    //        (const byte *)static_pub_key_bytes.c_str(),
    //        static_pub_key_bytes.size());
}

void dh_key_agreement::set_ephemeral_key_pairs(
    const std::string &eph_priv_key_bytes,
    ...
    //        (const byte *)eph_priv_key_bytes.c_str(),
    //        eph_priv_key_bytes.size());
    //    epub
    //        = boost::make_unique<CryptoPP::SecByteBlock>((const byte
    //        *)eph_pub_key_bytes.c_str(), eph_pub_key_bytes.size());
}
