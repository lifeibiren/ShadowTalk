...
namespace sml {
configuration::configuration(const sml::address &server, const std::string &id,
...
configuration::configuration(const YAML::Node &node) {
    ...
    trusted_peer_key_map_
        = node["trusted_peers"].as<trusted_peer_key_map_type>();
}
...
const configuration::trusted_peer_key_map_type &
configuration::trusted_peer_key_map() const {
    return trusted_peer_key_map_;
}

void configuration::add_trusted_peer_public_key(
    const std::string &id, const std::string &public_key) {
    trusted_peer_key_map_[id] = public_key;
}

const std::string configuration::get_peer_public_key_by_id(
    const std::string &id) const {
    trusted_peer_key_map_type::const_iterator it
        = trusted_peer_key_map_.find(id);
