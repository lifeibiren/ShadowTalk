...
UdpService::UdpService(unsigned short port)
    ...
    , thread_ptr_(new boost::thread(boost::bind(
          &boost::asio::io_service::run, &io_service_, thread_err_))) {
    server_.register_receive_handler(
        boost::bind(&UdpService::receive_handler, this, _1, _2, _3));
    server_.register_send_handler(
        boost::bind(&UdpService::send_handler, this, _1, _2, _3));
}
void UdpService::send_message(boost::shared_ptr<message> msg) {
    ...
    if ((it = peer_endpoint_bm_.right.find(msg->dst_peer()))
        == peer_endpoint_bm_.right.end()) {
        boost::asio::ip::udp::endpoint remote_endpoint(
            boost::asio::ip::address::from_string(msg->dst_peer()->address()),
            msg->dst_peer()->port());
        peer_endpoint_bm_.insert(peer_endpoint_bm_type::value_type(
            remote_endpoint, msg->dst_peer()));
...
void UdpService::receive_handler(boost::shared_ptr<std::string> bytes,
    boost::asio::ip::udp::endpoint &remote_endpoint,
    ...
    if ((it = peer_endpoint_bm_.left.find(remote_endpoint))
        == peer_endpoint_bm_.left.end()) {
        boost::shared_ptr<peer> new_peer(new peer(
            remote_endpoint.address().to_string(), remote_endpoint.port()));
        peer_endpoint_bm_.insert(
            peer_endpoint_bm_type::value_type(remote_endpoint, new_peer));
...
void UdpService::send_handler(boost::shared_ptr<std::string> bytes,
    boost::asio::ip::udp::endpoint &remote_endpoint,
    const boost::system::error_code &error) {
